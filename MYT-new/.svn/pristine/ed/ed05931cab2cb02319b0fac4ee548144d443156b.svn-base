...
public class MybatisGenerateUtils {
	...
	// 工程路径以\\结束
	private static String project_url = "F:\\work\\MYT\\src\\"; // 工程路径以\\结束
	private static String mbt_xml_package = ""; // 配置文件路径
	private static String bean_package = "com\\yihu\\myt\\vo"; // 实体路径
	private static String service_package = "com\\yihu\\myt\\service\\service"; // dao路径
	private static String dao_package = "com\\yihu\\myt\\dao"; // dao路径
	private static String sqlNameEnum = "com\\yihu\\myt\\enums"; // 枚举路径
	private static String service_end_sign = "Service"; // DAO以什么结尾
	private static String encode = "GBK";
	


	//private static String url = "jdbc:sqlserver://10.0.200.7:1433;database=baseinfo";
	//private static String url = "jdbc:sqlserver://10.0.200.6:1433;database=Bespeak";
	private static String url = "jdbc:sqlserver://10.0.200.1:1433;database=BOSS";
	/*private static String user = "sa";
	private static String psd = "1qaz@wsx";
	private static String user = "sysgreen";
	private static String psd = "sysgreen*()jkzl";*/
	private static String user = "boss";
	private static String psd = "boss_admin";
	
	private static String driver = "com.microsoft.sqlserver.jdbc.SQLServerDriver";
	private static String table = "ght_numberwater";
	private static String table_text = "NumberWater";
	...
	public static void geneInteface() {
		File file = new File(project_url + service_package + "/I" + getFormatTableName() + "Service.java");
		System.out.println(file.getPath());
		File parentFile = file.getParentFile();
		if (!parentFile.exists()) {
			parentFile.mkdirs();
	...
	public static void geneImpl() throws SQLException {
		File file = new File(project_url + service_package + "/impl/" + getFormatTableName() + "Service.java");
		System.out.println(file.getPath());
		File parentFile = file.getParentFile();
		if (!parentFile.exists()) {
			parentFile.mkdirs();
		...
		dao.append("import com.yihu.myt.dbutil.MyDatabaseEnum;\r");
		dao.append("import com.yihu.myt.dbutil." + getFormatTableName() + "SqlNameEnum;\r");
		dao.append("import com.yihu.utils.StringUtil;");

		dao.append("import " + getBeanPackage() + "." + getFormatTableName() + "Vo;\r"); // 导入bean类
		dao.append("import " + getDAOPackage() + ".I" + getFormatTableName() + service_end_sign + ";\r"); // 导入bean类

		dao.append("\r\r\n");
		dao.append("public class " + getFormatTableName() + service_end_sign + " implements I" + getFormatTableName() + service_end_sign + "{\r\r\n");
		dao.append(getFormatTableName() + "DAO " + lowerFirestChar(getFormatTableName()) + "DAO = new " + getFormatTableName() + "DAO();\r\r\n");
		...
		dao.append("Integer count = DB.me().queryForInteger(MyDatabaseEnum.boss, sql);\r\t\t");
		...
		dao.append("List<" + getFormatTableName() + "Vo" + "> list = DB.me().queryForBeanList(MyDatabaseEnum.boss, sql," + getFormatTableName() + "Vo.class" + ");\r\t\t");
		...
		dao.append("JdbcConnection conn = DB.me().getConnection(MyDatabaseEnum.boss);\r\t\t");
		dao.append("try {\r\t\t");
		dao.append("conn.beginTransaction();\r\t\t");
		dao.append(lowerFirestChar(getFormatTableName()) + "DAO.insert" + getFormatTableName() + "(vo,conn);\r\t\t");
		dao.append("conn.commitTransaction(true);\r\t\t");
		dao.append("} catch (Exception e) {\r\t\t");
		dao.append("conn.rollbackAndclose();\r\t\t");
		dao.append("throw e;}}\r\t\t");

		// 更新
		dao.append("\t").append("/**").append("\r\t*添加\r").append("\t*/").append("\r");
		methodName = "insert" + getFormatTableName();
		dao.append("\t").append("public void update" + getFormatTableName() + "(" + getFormatTableName() + "Vo vo) throws Exception{\r\t\t");
		dao.append("JdbcConnection conn = DB.me().getConnection(MyDatabaseEnum.boss);\r\t\t");
		dao.append("try {\r\t\t");
		dao.append("conn.beginTransaction();\r\t\t");
		dao.append(lowerFirestChar(getFormatTableName()) + "DAO.update" + getFormatTableName() + "ByCondition(vo,conn);\r\t\t");
		dao.append("conn.commitTransaction(true);\r\t\t");
		dao.append("} catch (Exception e) {\r\t\t");
		dao.append("conn.rollbackAndclose();\r\t\t");
		dao.append("throw e;}}\r\t\t");
	 ...
	 * @throws SQLException
	 */
	public static void geneDAO() throws SQLException {
		File file = new File(project_url + dao_package + "/impl/" + getFormatTableName() + "DAO.java");
		System.out.println(file.getPath());
		File parentFile = file.getParentFile();
		if (!parentFile.exists()) {
			parentFile.mkdirs();
		...
		dao.append("\r\r\n");
		dao.append("public class " + getFormatTableName() + "DAO {\r\r\n");

		// 添加
		dao.append("\t").append("/**").append("\r\t*添加\r").append("\t*/").append("\r");
		String methodName = "insert" + getFormatTableName();
		dao.append("\t").append("public void insert" + getFormatTableName() + "(" + getFormatTableName() + "Vo vo,JdbcConnection conn) throws Exception{\r\t\t");
		dao.append("Sql sql = DB.me().createSql(" + getFormatTableName() + "SqlNameEnum." + methodName + ");\r\t\t");
		// 生成的get/set
		...
		dao.append("DB.me().insert(conn,sql);\r\t\t");
		dao.append("}").append("\r\r\n");
	...
	public static String getIntefaceContent() {
		...
		dao.append("import " + getBeanPackage() + "." + getFormatTableName() + "Vo;\r"); // 导入bean类
		dao.append("import " + getServicePackage() + ".impl." + getFormatTableName() + service_end_sign + ";\r"); // 导入impl类

		dao.append("\r\r\n");
		dao.append("@By(" + getFormatTableName() + service_end_sign + ".class)").append("\r\n");
		dao.append("public interface I" + getFormatTableName() + service_end_sign + "{\r\r\n");
		...
		if (keyList.size() > 0) {
			dao.append("\t").append("/**").append("\r\t*修改\r").append("\t*/").append("\r");
			dao.append("\t").append("public void update" + getFormatTableName() + "(" + getFormatTableName() + "Vo vo) throws Exception;").append("\r\r\n");
		}
		dao.append("}\r");
		return dao.toString();
	}
	...
	public static ResultSetMetaData getResultSetMetaData(String sql) {
		...
		try {
			...
			System.out.println("获取ResultSetMetaData失败...在" + MybatisGenerateUtils.class.getName() + ".ResultSetMetaData()方法中");
			e.printStackTrace();
		}
		return rsmd;
	}
